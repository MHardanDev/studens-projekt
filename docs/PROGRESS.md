...
تم تنفيذ المراحل 0 حتى 9، ثم تنفيذ المرحلة 10 لـSEO أساسي وقياس داخلي محدود ضمن نطاق الإشراف. لا توجد مفضلة أو معاينات تلقائية أو إرسال بريد حقيقي أو إعلانات أو analytics خارجية.
...
| 9 | مكتملة محلياً مع عوائق إطلاق معلنة | مصدر وحقوق وإقرار ثابت، مراجعة حقوق، بلاغات محدودة، حجب عبر archived، وأربع صفحات ثقة | migration، `check`, `test` (77 اختباراً), `ruff` | النصوص القانونية والترجمات وهوية المشغل والتواصل والاحتفاظ تحتاج قرارات ومراجعة قبل الإطلاق |
| 10 | مكتملة محلياً ضمن النطاق المطلوب | meta وcanonical وOpen Graph وJSON-LD، sitemap وrobots/noindex، عداد تنزيل كامل، ومؤشرات إشرافية scoped | migration، `check`, `test` (86 اختباراً), `ruff` | النطاق النهائي وتهيئة proxy وسياسة العداد تحتاج حسم؛ الإعلانات غير موجودة وتحتاج موافقة منفصلة |
...
## ما تغير في المرحلة 9

- أضيفت حقول صاحب المحتوى والمصدر وأساس السماح بالنشر إلى `StudyDocument` ولقطة `DocumentVersion`.
- أضيف إقرار مطلوب إلى نموذج الرفع، ويحفظ نصه وتاريخه والحساب الذي قبله بدلاً من حفظ checkbox مجردة.
- صار الإذن الصريح يحتاج دليلاً خاصاً والترخيص يحتاج اسمه وشروطه، مع منع الاعتماد إذا بقيت بيانات الحقوق ناقصة.
- لم تضف migration قيماً للسجلات القديمة؛ تظهر نواقصها للمراجع وللجمهور بصيغة `غير موثق لملف قديم` دون اختلاق إذن.
- أضيفت بيانات الحقوق إلى صفحة مراجعة الملفات، مع إبقاء دليل الإذن والتواصل ونص الإقرار خارج الصفحات العامة.
- وسع `DocumentReport` القائم بملاحظات المراجع وهوية المراجع وتاريخه، وحسن تسمية سبب انتهاك حقوق النشر.
- أضيف حد إرسال مستقل قابل للضبط لبلاغات الملفات، مع بقاء POST محمياً بـCSRF.
- أضيفت صفحة `/ar/moderation/reports/` لمعالجة البلاغ ضمن `ModeratorScope` وإغلاقه أو رفضه أو حجب الوثيقة عبر `archived`.
- يمنع الحجب صفحة الوثيقة والتنزيل عبر `GET`, `HEAD`, و`Range`، ولا يدعي استرجاع النسخ التي سبق تنزيلها.
- أضيفت صفحات عن الموقع وإرشادات المشاركة والخصوصية والتواصل والإبلاغ، وربطت بالتذييل وتحت بادئات اللغات الثلاث.
- النصوص مسودات عربية تصف السلوك الفعلي، ولا تحتوي اسماً قانونياً أو بريداً أو عنواناً أو مدة احتفاظ مختلقة.
- أضيفت 10 اختبارات للحقوق والإقرار والخصوصية والبلاغات والنطاق والحجب وصفحات الثقة واللغات.

## ما تغير في المرحلة 10

- أضيف عنوان ووصف وcanonical وOpen Graph للصفحات العامة، بلا صور أو خطوط أو scripts خارجية.
- أضيف JSON-LD دقيق وخفيف للرئيسية والتصفح وصفحات المادة والملف والثقة، دون تقييمات أو جهة مشغلة مختلقة.
- أضيف `/sitemap.xml` للصفحات العامة والمواد ذات محتوى منشور والوثائق المعتمدة النظيفة فقط، مع مسارات اللغات الثلاث.
- أضيف `/robots.txt` وmiddleware لترويسة `X-Robots-Tag` وmeta noindex للحسابات والرفع والإدارة والإشراف والطلبات والبلاغات والتنزيل ونتائج البحث.
- أضيف `full_download_count` إلى النسخة المنشورة، ويزيد ذرياً عند بدء `GET` كامل ناجح بلا `Range` فقط.
- لا يخزن العداد IP أو User-Agent أو fingerprint أو cookie أو نص بحث، ولا يحتسب `HEAD` أو طلبات `Range`.
- أضيفت صفحة مؤشرات للمشرف والمدير تعرض الملفات المنشورة والتنزيلات والبلاغات ضمن `ModeratorScope` فقط، دون بيانات تواصل البلاغ.
- حدثت مسودة الخصوصية لتعكس العدادات الداخلية الفعلية وعدم وجود tracking خارجي.
- لم يضف أي كود إعلانات أو analytics؛ سجلت قائمة قرارات الإعلانات المستقبلية في الأسئلة المفتوحة.
- أضيفت 9 اختبارات لـSEO والفهرسة والعداد وصلاحيات المؤشرات، فأصبح الإجمالي 86 اختباراً.
...
- `.venv\Scripts\python manage.py makemigrations catalog`: أنشأ migration `catalog.0006` للمرحلة 9 دون data migration تملأ حقوقاً قديمة.
- `.venv\Scripts\python manage.py migrate --noinput`: طبق migration المرحلة 9 على SQLite المحلي.
- اختبارات مسارات الرفع والمراجعة والتنزيل بعد إضافة شرط الحقوق: نجحت 31 اختباراً.
- اختبارات المرحلة 9 المعزولة: نجحت 10 اختبارات.
- `.venv\Scripts\python manage.py test`: نجحت 77 اختباراً بعد المرحلة 9.
- `.venv\Scripts\python -m ruff check .`: نجح بعد المرحلة 9.
- `.venv\Scripts\python manage.py makemigrations --check --dry-run`: لم يجد تغييرات نماذج غير مولدة بعد المرحلة 9.
- فحص HTTP محلي: الرئيسية والتصفح وصفحات الثقة العربية و`/en/privacy/` و`/de/privacy/` و`/health/` رجعت `200`، وصفحة `/ar/moderation/reports/` أعادت الزائر إلى تسجيل الدخول بـ`302`.
- `.venv\Scripts\python manage.py makemigrations catalog`: أنشأ `catalog.0007` لحقل عداد التنزيل الكامل على `DocumentVersion`.
- `.venv\Scripts\python manage.py migrate --noinput`: طبق migration المرحلة 10 على SQLite المحلي.
- اختبارات المرحلة 10 المعزولة: نجحت 9 اختبارات.
- `.venv\Scripts\python manage.py test`: نجحت 86 اختباراً بعد المرحلة 10.
- `.venv\Scripts\python -m ruff check .`: نجح بعد المرحلة 10.
- `.venv\Scripts\python manage.py makemigrations --check --dry-run`: لم يجد تغييرات نماذج غير مولدة بعد المرحلة 10.
- فحص HTTP محلي: `/ar/`, `/ar/browse/`, `/robots.txt`, `/sitemap.xml`, و`/health/` رجعت `200`، وصفحة المؤشرات أعادت الزائر إلى تسجيل الدخول.
...
- لم تراجع مسودات صفحات الثقة قانونياً، ولم تحدد هوية المشغل أو بلد التشغيل أو قناة التواصل العامة أو مدد الاحتفاظ.
- لم تنشأ ترجمات إنكليزية وألمانية كاملة؛ المسارات والاتجاه يعملان، لكن النص العربي يبقى fallback الحالي.
- لم يختبر وصول أدلة الإذن عبر تخزين ملفات مرفقة؛ الدليل الحالي حقل نص خاص فقط، وتحديد صيغة الإثبات المقبولة قرار قبل الإطلاق.
- لم يختبر حد البلاغات عبر cache مشتركة أو عدة عمليات خادم؛ الاختبار يستخدم `LocMemCache`.
- لم يختبر استرجاع نسخة احتياطية بعد حجب ملف؛ يلزم runbook يضمن إعادة تطبيق سجل الحجب قبل الإطلاق.
- لم تنفذ قناة طلب حذف الحساب أو بيانات التواصل بعد لعدم وجود مشغل وقناة عامة معتمدين.
- لم يعتمد نطاق عام نهائي أو إعداد HTTPS/reverse proxy، لذلك لم تختبر canonical وsitemap على اسم نطاق إنتاج فعلي.
- عداد التنزيل يقيس بدء استجابة GET كاملة ولا يثبت وصول آخر بايت إلى العميل؛ لم يضف تتبع إكمال لأنه يتطلب تعقيداً وقياساً إضافياً غير مبرر.
- لم تختبر صفحة المؤشرات على PostgreSQL أو مع تحميل متزامن؛ اختبرت الاستعلامات والصلاحيات على SQLite المحلي.
- لم تختبر sitemap في محرك بحث حقيقي أو أدوات مشرفي المواقع لأن الموقع لم ينشر للعامة.
- لم تدمج إعلانات أو analytics أو scripts خارجية عمداً؛ أي اختبار لها مؤجل إلى قرار وموافقة منفصلين.

## الخطوة التالية

الخطوة التالية عند طلبك الصريح هي المرحلة 11 فقط. قبل الإنتاج ما زال يلزم حسم الأسئلة القانونية والتشغيلية والنطاق النهائي، واختبار PostgreSQL وClamAV/worker وcache مشتركة وتخزين/تسليم الملفات في بيئة staging، وتجربة المسارات على اتصال ضعيف وجهاز Android حقيقي.
