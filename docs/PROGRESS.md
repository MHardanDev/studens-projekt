...
تم تنفيذ المراحل 0 حتى 6، ثم تنفيذ المرحلة 7 لصفحة الملف العامة والتنزيل الخاص المتدفق ووضع توفير البيانات واستئناف آخر مكان. لا توجد معاينات تلقائية أو إرسال بريد حقيقي أو إعلانات.
...
| 8 | غير منفذة | لا مفضلة أو طلبات محتوى بعد | غير منفذ | تعتمد على الحسابات والفهرس |
...
- `.venv\Scripts\python manage.py test`: نجحت 52 اختباراً بعد المرحلة 7.
...
- اختبارات المرحلة 7 المعزولة: نجحت 10 اختبارات، ومنها `HEAD`, `206`, `416`, و`If-Range` على ملف اختبار ذي توقيع PDF.
...
- لم تختبر إرسال بريد أو Redis أو RQ أو رفع ملفات أو صلاحيات مساهمين لأنها خارج نطاق المرحلتين 1 و2.
...
- لم تضف صفحات تفاصيل المادة أو الملف أو تنزيل الملفات لأنها خارج نطاق المرحلة 3 الحالية.
- لم يختبر إرسال بريد حقيقي للدعوات لأنه غير منفذ عمداً في المرحلة 4.
- لم يختبر MFA لأنه مؤجل.
- لم تختبر صلاحيات رفع أو مراجعة أو نشر ملفات لأنها لم تبدأ بعد.
...
- لم يختبر تنزيل أو معاينة ملفات مرفوعة لأنها غير منفذة عمداً قبل المراحل التالية.
...
الخطوة التالية عند طلبك الصريح هي المرحلة 8 فقط. قبل الإنتاج ما زال يلزم اختبار PostgreSQL وClamAV/worker وتخزين/تسليم الملفات في بيئة staging، وتجربة تنزيل PDF حقيقي كبير وعلى اتصال ضعيف.
